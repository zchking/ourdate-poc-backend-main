...
class Event(models.Model):
    ...
    registration_limit = models.IntegerField('Guest limit', default=0, choices=[
                                             (0, u"No limit")] + list(zip(range(1, 1000), range(1, 1000))))
...
def EventRegistration(request, event_id):
    event = models.ManyToOneRel(Event, blank=True, on_delete=models.CASCADE)
    user = models.ManyToOneRel(User, blank=True, on_delete=models.CASCADE)
...
def remove_user_from_list_of_attendees(self, user):
    ...
    cancelation = CancelledRegistrations.objects.create(
        ...
        time_registered=EventRegistration.objects.get(
            user=user, event=self).time_registered
    )
    cancelation.save()
